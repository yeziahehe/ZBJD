//
//  DevelopByMonthViewController.m
//  ZBJD
//
...
@interface DevelopByMonthViewController ()
...
@implementation DevelopByMonthViewController
...
- (void)loadSubViewWithData
{
    ...
    for (NSInteger i = self.developArray.count - 1; i >= 0; i--) {
        ...
        if (![d.finish isEqualToString:@"0"]) {
            [xArray addObject:[d.developDate substringFromIndex:8]];
    ...
    tipLabel.text = @"未显示日期表示该日期用户发展量为0";
